...
export default function Page() {
  ...
  const [manualRows, setManualRows] = useState<WechatTransactionRow[]>([]);
  ...
  `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}:${String(d.getSeconds()).padStart(2, '0')}`;
    ...
    const row: WechatTransactionRow = {
      tradeTime,
      product,
      amount,
      amountRaw: amountInput.trim(),
    };
    setManualRows((prev) => [row, ...prev]); // 新的放最前面
    setTradeTimeInput("");
    setProductInput("");
    setAmountInput("");
    // 如果之前是 error，新增成功後把錯誤清掉
  ...
  const onPickFile: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    try {
      ...
      setState({ status: "ready", rows: parsed, filename: file.name });
    } catch (err) {
